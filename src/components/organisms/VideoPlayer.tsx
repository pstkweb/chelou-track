...
import { AlertCircle, Maximize2, Minimize2, Pause, Play } from 'lucide-react';
...
export default function VideoPlayer({
  ...
  return (
    ...
    >
      ...
      />
      {hasError ? (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-[rgba(0,0,0,.75)] px-10 text-center text-white">
          <AlertCircle size={28} className="text-red-400" />
          <p className="text-sm">Impossible de lire cette vidéo.</p>
          {platform() === 'linux' && (
            <p className="max-w-md text-[rgba(255,255,255,.65)] text-xs">
              Vérifiez que les paquets GStreamer de base sont installés sur votre système
              (gstreamer-plugins-base, -good, -bad, -ugly et gst-libav ou équivalents selon votre
              distribution).
            </p>
          )}
        </div>
