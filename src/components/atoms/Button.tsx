...
export default function Button({ variant, size, className = "", children, ...rest }: ButtonProps) {
  const cls = cn([
    ...
    variant === "primary" &&
      "bg-accent text-accentink border-transparent shadow-[0_6px_18px_-6px_var(--accent)] hover:brightness-[1.06] hover:border-transparent",
    variant === "ghost" &&
      "bg-transparent border-transparent hover:bg-chip hover:border-transparent",
