...
use crate::{Entry, StorageProvider};
...
pub async fn scan_methods_in_folder<P: StorageProvider>(
    client: &P,
    root_folder_id: &str,
    on_progress: Arc<dyn Fn(ScanEvent) + Send + Sync>,
) -> Result<Vec<Method>> {
...
#[async_recursion]
async fn scan_folder_contents<P: StorageProvider>(
    client: &P,
    ...
    let mut special_ids: HashSet<String> = HashSet::new();
